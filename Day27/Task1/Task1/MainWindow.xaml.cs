    ...
    public partial class MainWindow : Window
    {
        ...
        private void DisplayXmlContent()
        {
            ...
            if (rbTextBlock.IsChecked == true)
            {
                ...
                lstOutput.Visibility = Visibility.Collapsed;
                treeOutput.Visibility = Visibility.Collapsed;
            ...
            else if (rbListBox.IsChecked == true)
            {
                ...
                lstOutput.Visibility = Visibility.Visible;
                treeOutput.Visibility = Visibility.Collapsed;
                ...
                {
                    string lastName = student.SelectSingleNode("LastName")?.InnerText;
                    string firstName = student.SelectSingleNode("FirstName")?.InnerText;
                    string birthDate = student.SelectSingleNode("BirthDate")?.InnerText;
                    lstOutput.Items.Add($"{lastName} {firstName} ({birthDate})");
                }
            }
            else if (rbTreeView.IsChecked == true)
            {
                txtOutput.Visibility = Visibility.Collapsed;
                lstOutput.Visibility = Visibility.Collapsed;
                treeOutput.Visibility = Visibility.Visible;

                treeOutput.Items.Clear();
                XmlNode rootNode = xmlDoc.DocumentElement;
                if (rootNode != null)
                {
                    TreeViewItem rootItem = new TreeViewItem { Header = rootNode.Name };
                    treeOutput.Items.Add(rootItem);

                    foreach (XmlNode studentNode in rootNode.ChildNodes)
                    {
                        TreeViewItem studentItem = new TreeViewItem { Header = "Student" };
                        rootItem.Items.Add(studentItem);

                        foreach (XmlNode childNode in studentNode.ChildNodes)
                        {
                            TreeViewItem item = new TreeViewItem { Header = $"{childNode.Name}: {childNode.InnerText}" };
                            studentItem.Items.Add(item);
                        }
                    }
        ...
        {
            if (IsLoaded) // Ensure the window is loaded before trying to display content
            {
                DisplayXmlContent();
            }
        }
